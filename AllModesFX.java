 ...
 *
  * This class was created by: Javier Vasquez, David Hoefs and Zach Hunt
 */
package myproject3;
...
public class AllModesFX extends BangFXMain {
    ...
    AllModesFX(){
        ...
        nextPlayer.setOnAction((e -> {
            ...
            if(zombieHandCount > aliveInGame && aliveInGame > 1 && !outbreak){
                outbreak = true;
                boolean renZomb = false;
                System.out.println("OUTBREAK");
                cardsDrawnLabel.setText("ZOMBIE OUTBREAK!");
                for(MyPlayer player: players){
                    if(player.getHp() <= 0 && "Renegade".equals(player.getRole()) && !renZomb){
                        player.zombie = true;
                        player.setHp(player.getMaxHp() + alive);
                        renZomb = true;
            ...
            // Draw Undead Card
            else if(players.get(index).getHp() <=0 && !outbreak) {
                p1c.setText("");
                p2c.setText("");
                p3c.setText("");
                p4c.setText("");
                ...
                switch(this.index){
                    case 0:
                        p1c.setText("Player 1's Turn to Draw a Card");
                        break;
                    case 1:
                        p2c.setText("Player 2's Turn to Draw a Card");
                        break;
                    case 2:
                        p3c.setText("Player 3's Turn to Draw a Card");
                        break;
                    case 3:
                        p4c.setText("Player 4's Turn to Draw a Card");
                        break;
                    case 4:
                        p5c.setText("Player 5's Turn to Draw a Card");
                        break;
                    case 5:
                        p6c.setText("Player 6's Turn to Draw a Card");
                        break;
                    case 6:
                        p7c.setText("Player 7's Turn to Draw a Card");
                        break;
                    case 7:
                        p8c.setText("Player 8's Turn to Draw a Card");
                        break;
                }
                drawZombCard(index);
            }
    ...
    public void drawZombCard(int i){
    ...
    */
    ...
    public void roll(MyPlayer currPlayer){
        ...
        reRoll.setOnAction((e -> {
            ...
            if(duelDice1.isSelected() && !currPlayer.zombie){
            ...
            if(duelDice2.isSelected() && !currPlayer.zombie){
        ...
        saveRolls.setOnAction((e -> {
            ...
            if(beerCount>0 && !currPlayer.zombie){
                if(gameOver == false){
    ...
    public void updateHp(int i){
        ...
        switch(i){
            case 0:
                ...
                if(outbreak && players.get(i).zombie){
        ...
        for(MyPlayer player : players){
            ...
            if(player.zombie && player.getHp() > 0)
                zombieAlive++;
            else if(!player.zombie && player.getHp() > 0)
                survivorAlive++;
        }
        
        if(survivorAlive == 0 && outbreak){
            winner.setText("THE WINNER IS THE ZOMBIES!");
            winner.setStyle("-fx-text-fill: #00ffff");
            this.gameOver = true;
        }
        else if(survivorAlive >= 1 && zombieAlive == 0 && outbreak){
            winner.setText("THE WINNERS ARE THE SURVIVORS");
            winner.setStyle("-fx-text-fill: #00fff");
            this.gameOver = true;
        }
        else if(sheriffAlive == 0 && outlawAlive > 0 && !outbreak){
            winner.setText("THE WINNER IS THE OUTLAWS");
            winner.setStyle("-fx-text-fill: #00ffff");
            disableAll();
            this.gameOver = true;
        }
        else if(outlawAlive == 0 && renegadeAlive == 0 && !outbreak){
            winner.setText("THE WINNER IS THE SHERIFF");
            winner.setStyle("-fx-text-fill: #00ffff");
            disableAll();
            this.gameOver = true;
        }
        else if(outlawAlive == 0 && sheriffAlive == 0 && deputyAlive == 0 && renegadeAlive == 1 && !outbreak){
    ...
    @Override
     public void AIroll(MyPlayer currPlayer){
        ...
        while(duelCount > 0){
            ...
            while(players.get(n) == currPlayer || players.get(n).getHp() <= 0)
        ...
        while(whiskeyCount > 0 && currPlayer.getHp() > 0 && gameOver == false && !currPlayer.zombie) {
            if(currPlayer.zombie == false){
                if("Greg Digger".equals(currPlayer.getCharacterName()))
                currPlayer.gainHp(2);
            else
    ...
    */
    public void duelLogic(int i, int j){
        ...
        if(roll == 4 || roll == 5)
            ...
            //    dropArrow(loser);
            if("El Gringo".equals(loser.getCharacterName()))
                arrow(players.get(i));
        }
    }
